...
var _ = Describe("Creating a new handler with attributes", func() {
	...
	BeforeEach(func() {
		...
		handler := slogctx.NewHandler(slog.NewTextHandler(buf, nil), noopGetter).
			WithAttrs([]slog.Attr{fooAttr, barAttr, pifAttr})
		slog.New(handler).Info(testMessage)
	})
	...
	When("a message is logged", func() {
		...
		Specify("the output includes the attributes", func() {
			Expect(buf.String()).To(
				And(
					ContainSubstring(fooAttr.String()),
					ContainSubstring(barAttr.String()),
					ContainSubstring(pifAttr.String()),
				),
			)
		})
	})
